...
use xdoc::{Declaration, Document, Element, Encoding, Misc, Node, Version, PI};
...
fn main() {
    ...
    let root = Element {
        ...
        nodes: vec![
            Node::Misc(Misc::PI(PI {
                target: "d".to_owned(),
                instructions: vec!["e".to_owned()],
            })),
            ...
            Node::Misc(Misc::PI(PI {
                target: "g".to_owned(),
                instructions: vec!["h".to_owned()],
            })),
        ],
    };
    doc.push_epilog_misc(Misc::PI(PI {
        target: "i".to_owned(),
        instructions: vec!["j".to_owned()],
    }));
    doc.set_root(root);
